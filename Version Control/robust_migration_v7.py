...
DB_FILE = 'library_polished_v7.db'

# --- 1. IMPROVED PUBLICATION SPLITTER ---
def split_publication_info(full_str):
    """
    Robust splitter that handles:
    1. "NONE S.CHAND AND CO., 1984" (Comma was for date)
    2. "NEW DELHI, NEW AGE..." (Comma is for place)
    3. "NEWDELHI WHEELER" (Space separator)
    """
    ...
    # B. CRITICAL FIX: Clean trailing punctuation left by the date
    # "NONE S.CHAND AND CO.," -> "NONE S.CHAND AND CO."
    clean_str = clean_str.rstrip('.,;: ')

    place = None
    publisher = None

    # C. Strategy 1: Explicit "NONE" Handling
    # If it starts with "NONE ", that is definitely the place.
    if clean_str.startswith("NONE ") or clean_str.startswith("none "):
        place = "NONE"
        publisher = clean_str[5:].strip() # Slice off "NONE "
        
    # D. Strategy 2: Standard MARC Colon Separator (City : Publisher)
    elif ':' in clean_str:
        parts = clean_str.split(':', 1)
        place = parts[0].strip()
        publisher = parts[1].strip()

    # E. Strategy 3: Comma Separator (City, Publisher)
    elif ',' in clean_str:
        parts = clean_str.split(',', 1)
        place = parts[0].strip()
        publisher = parts[1].strip()
        
    # F. Strategy 4: Fallback to First Space
    else:
...
# --- [Keep Parsing Helpers Identical to V6] ---
def heal_scientific_notation(text):
    if not text: return text
    pattern = re.compile(r'(\d\.\d+)[Ee]\+(\d+)')
    def replace_match(match):
        try:
            return str(int(float(match.group(1)) * (10 ** int(match.group(2)))))
...
def run_polished_migration():
    print(f"Initializing Polished Database: {DB_FILE}...")
    ...
    with open(INPUT_FILE, 'r', encoding='utf-8') as f:
        ...
        if batch_biblio:
             ...
             conn.commit()
    conn.close()
    print("Polished Migration Complete. Check library_polished_v7.db")

if __name__ == "__main__":
    run_polished_migration()